  ...
  return (
    <AdminLayout title="Restaurant Settings">
      <div className="settings-card">
        <div className={`store-status-box ${form.is_open ? "open" : "closed"}`}>
          <div>
            <h2>{form.is_open ? "Restaurant is Open" : "Restaurant is Closed"}</h2>
            <p>
              {form.is_open
                ? "Customers can view the QR menu and place orders."
                : "QR menu will show a closed message and block new orders."}
            </p>
          </div>

          <button
            type="button"
            className={`store-toggle-btn ${form.is_open ? "open" : "closed"}`}
            onClick={() => update("is_open", !form.is_open)}
          >
            {form.is_open ? "Close Store" : "Open Store"}
          </button>
        </div>
